...
def create_yolo_dets(bboxes: np.asarray, labels: list, width, height, class_name={}):
    """
...
def create_yolo_segs(bboxes: np.asarray, labels: list, width, height, class_name={}):
    """
    :param bboxes: [[xmin,ymin,xmax,ymax]]
    :param labels:[name1,name2]
    :param keypoint:
    ...
    for i in range(len(labels)):
        ...
        item = [label] + [xmin, ymin, xmax, ymin, xmax, ymax, xmin, ymax]
        contents.append(item)
    return contents
...
def convert_voc2yolo_segment(data_root, out_text_dir, out_image_dir=None, class_name=None, rename="", vis=True):
    """
    将VOC的(xmin,ymin,xmax,ymax)转换为YOLO格式数据(class,cx,cy,w,h)/(1,width,height,width,height)
    :param data_root: VOC数据根目录
    :param out_text_dir: output VOC XML
...
if __name__ == "__main__":
    ...
    convert_voc2yolo.convert_voc2yolo_segment(data_root, out_text_dir, class_name=class_name, vis=False)
