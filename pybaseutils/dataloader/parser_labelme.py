...
class LabelMeDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        """
        image_id = self.index2id(index)
        image_file, json_file, image_id = self.get_image_anno_file(image_id)
        image = self.read_image(image_file, use_rgb=self.use_rgb)
        shape = image.shape
        annotation = self.load_annotations(json_file)
...
def maker_labelme(json_file, annotation, image_size, image_file="", image_bs64=None):
    """
    制作label数据格式
    :param json_file: 保存json文件路径
    :param annotation: {"point": point, "label": label}等数据
    :param image_size: (W,H)
    :param image_file: 图片路径，如果存在则进行拷贝到json_file同一级目录
    :param image_bs64: 图片base64编码，可为None
    :return:
    """
    file_utils.create_file_path(json_file)
    shapes = []
    for point, label in zip(annotation['point'], annotation['label']):
    ...
    if os.path.exists(image_file): file_utils.copy_file_to_dir(image_file, os.path.dirname(json_file))
...
if __name__ == "__main__":
    ...
    for i in range(len(dataset)):
        ...
        json_file = os.path.join("masker", "{}.json".format(os.path.basename(image_file).split(".")[0]))
        maker_labelme(json_file, data, (w, h), image_file=image_file, image_bs64=None)
