...
def demo_vis_CocoInstances():
    ...
    #               '电能表': 13, '尖嘴钳': 14, '验电笔': 15, '螺丝刀': 16, '接线盒': 17, '电流互感器': 18, '表箱关': 19,
    #               '表箱开': 19, '竹梯': 20, '准备区域': 21, '工作台': 22}
    class_name = ["BG", '遮拦杆', '主杆', '垫子', '柱式绝缘子', '抹布', '吊物绳', '绝缘手套', '脚扣', '安全帽',
                  '导线头', '尖嘴钳',
                  '扳手', '螺丝', '铁架', '身穿工作服', '安全带', '绝缘鞋', '工具袋', '铝扎线', '止步高压危险标示牌',
                  '从此进出标示牌', '在此工作标示牌', '手', '其他鞋', '安全绳', '未穿工作服']

    class_name = ['手', '未穿工作服', '身穿工作服', '其他鞋', '绝缘鞋', '安全带', '安全帽', '安全绳', '垫子',
                  '绝缘手套',
                  '主杆', '柱式绝缘子', '抹布', '吊物绳', '脚扣', '尖嘴钳', '扳手', '螺丝', '铁架', '工具袋', '铝扎线',
                  '导线头',
                  '遮拦杆', '止步高压危险标示牌', '从此进出标示牌', '在此工作标示牌', ]
    class_name = ["尖嘴钳"]

    # anno_file = "../data/coco/coco_ins.json"
    anno_file = "/home/PKing/Downloads/labeled_trainval/SSLAD-2D/labeled/val/instance_val.json"
    class_name = None
    dataset = parser_coco_ins.CocoInstances(anno_file=[anno_file], image_dir="", decode=True,
    ...
    for i in range(len(dataset)):
        ...
        points = data["segs"]  # 目轮廓
        # points = [image_utils.find_minAreaRect(seg)[0] for seg in points]  # OBB轮廓
