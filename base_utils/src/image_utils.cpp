...
cv::Mat hwc2chw(cv::Mat &src) {
    cv::Mat tmp;
    if (src.depth() == CV_8U) {
        src.convertTo(tmp, CV_32FC(src.channels()), 1.0 / 255.0);
    } else {
        tmp = src;
    }
    // 确保tmp是连续的，以便进行指针运算
    ...
    // 使用 OpenCV 的 split 和 merge 组合
    std::vector<cv::Mat> channels(tmp.channels());
    ...
    cv::Mat dst;
    cv::vconcat(channels, dst); // 垂直拼接
    return dst.reshape(1, {tmp.channels(), tmp.rows, tmp.cols});
}
...
cv::Mat hwc2chw_v2(cv::Mat &src) {
    ...
    int W = tmp.cols;
    int C = tmp.channels();
    cv::Mat dst(C, H * W, CV_32F);
    const float *sprt = tmp.ptr<float>(0);
    for (int c = 0; c < C; c++) {
        float *dptr = dst.ptr<float>(c);
        for (int i = 0; i < H * W; i++) {
            // 从交错内存读取，写入连续平面
            dptr[i] = sprt[i * C + c];
        }
    }
    //dst = dst.reshape(1, {C, H, W});
    int sizes[] = {C, H, W};
    dst = dst.reshape(1, 3, sizes);
    return dst;
}


cv::Mat hwc2nchw(cv::Mat &src, int n) {
    int H = src.rows;
    int W = src.cols;
    int C = src.channels();
    cv::Mat dst = hwc2chw(src);
    return dst.reshape(1, {n, C, H, W});
}
