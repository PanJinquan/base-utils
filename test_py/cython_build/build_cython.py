...
def build_app_modules(root,
    ...
    """
    获得项目需要编译的模块
    :param app_root: 项目APP路径，*.py源码根目录
    :param build_exclude: 不需要处理的文件
    :return: modules
    """
    app_name = os.path.basename(root)
    app_build = os.path.join(build_root, app)
    file_utils.copy_dir(root, build_root, exclude=exclude_files)
    ...
    for file in files:
        ...
        if file.endswith('__init__.py') or (file in exclude_modules):
            continue
        src = os.path.join(build_root, file)
        dst = os.path.join(build_root, file + "x")
        shutil.copyfile(src, dst)
        modules.append(file[:-3])
    clear_app_files(app_build, delete_types=['.c', '.py', '.so'], exclude_file=exclude_modules)  # 只保留pyx文件
    setup_file = gen_app_setup_file(os.path.join(build_root, 'setup.py'), app_name, modules)
    return setup_file, modules


def clear_app_files(app_root, delete_types, exclude_file):
    """
    删除编译中间文件
    :param app_root:
    :param delete_types:
...
if __name__ == '__main__':
    ...
    setup_file, modules = build_app_modules(root, app="app", build_root=build_root, exclude_modules=exclude_file)
    os.system(f'cd {build_root} && pwd && python3 {setup_file} build_ext --inplace')
    # clear_app_files
    # clear_app_files(build_root, delete_types=['.pyx', '.c'], exclude_file=exclude_file)  # 删除编译中间文件
    clear_app_files(build_root, delete_types=['.pyx', '.c', '.py'], exclude_file=exclude_file)  # 删除py文件和编译中间文件，用于部署
    # clear_app_files(build_root, delete_types=['.c', '.py'], exclude_file=exclude_file)  # 删除py文件和编译中间文件，用于部署
    # clear_app_files(build_root, delete_types=['.so', '.pyx', '.c'], exclude_file=exclude_file)  # 删除所有编译文件
