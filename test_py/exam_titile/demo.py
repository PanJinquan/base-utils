...
def read_exam_info(content, exam_info={}, title="单选题", start=["一、单选题"], end=["二、"]):
    """
    :param content:
    :return:
    """
    ...
    nums = len(content)
    record = False
    item_info = json_utils.get_value(exam_info, [title], default={})
    for i, line in enumerate(content):
        ...
        if file_utils.is_int(split[0]):  # 通过序列判断每道题
            if key and item_list: item_info[key] = item_list
...
def save_exam_info(data_root, exam_info):
    ...
    tmp = "tmp.txt"
    num = 0
    for title, item_info in exam_info.items():
        ...
        for item, content in item_info.items():
            count += 1
            content[0] = "{}、".format(count) + content[0]
            content[-1] = content[-1] + "\n"
...
if __name__ == '__main__':
    # data_root = "/home/PKing/Downloads/中医规培-公共科目-方剂学-1603"
    data_root = "/home/PKing/Downloads/中医规培-公共科目-内经-502"
    # data_root = "/home/PKing/Downloads/中医规培-公共科目-温病学-1058"
    ...
    for file in text_list:
        ...
        exam_info = read_exam_info(content, exam_info, title="单选题", start=["一、单选题"], end=end)
        # exam_info = read_exam_info(content, exam_info, title="多选题", start=["一、多选题"], end=end)
