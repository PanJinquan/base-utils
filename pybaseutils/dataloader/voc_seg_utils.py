...
class VocSeg(object):
    ...
    def get_segmentation_area(seg_path, bbox):
        ...
        seg = VocSeg.getsegmentation(seg_path, bbox)
        if seg:
            seg = [list(map(float, seg))]
            contour = VocSeg.change_format(seg[0])
            # 计算轮廓面积
            ...
            if flag:
                for i in range(mean_x, start, -1):
                    ...
                    if pixels != 0 and pixels != 220:
                        mask = (mask == pixels).astype(np.uint8)
                        break
            polygons = VocSeg.mask2polygons(mask)
            return polygons
        except:
            return []
    ...
    def getbbox(height, width, points):
        ...
        polygons = points
        mask = VocSeg.polygons_to_mask([height, width], polygons)
        return VocSeg.mask2box(mask)
