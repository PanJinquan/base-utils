...
def set_logger(name=None, level="debug", logfile=None, format="simple", is_main_process=True):
    ...
    if is_main_process:
        ...
        if logfile: logger.add(logfile, level=level.upper(), rotation="1 day", retention="7 days", format=format)
...
def set_logger_v2(name=None, level="debug", logfile=None, format="simple", is_main_process=True):
    ...
    if is_main_process:
        ...
        # 输出到文件，文件名app.log
        h2 = {"sink": logfile,
              "format": format, "colorize": False, "level": level.upper(), "rotation": "1 day",
              "retention": "7 days"} if logfile else None
